...
MODEL_MAPPING_NAMES: "OrderedDict[str, tuple[str, str]]" = OrderedDict(
    [
        ...
        ("audio-clap", ("auden.models.audio_clap.model", "AudioClapModel")),
...
class _LazyModelMapping(OrderedDict[str, Type[nn.Module]]):
    ...
    def __getitem__(self, key: str) -> Type[nn.Module]:
        ...
        if key not in self._mapping:
            raise KeyError(f"Model type '{key}' not found. Available models: {list(self.keys())}")
    ...
    def keys(self) -> list[str]:
        return list(self._mapping.keys()) + list(_DYNAMIC_MODELS.keys()) + list(self._extra_content.keys())
    ...
    def __contains__(self, item: object) -> bool:
        return item in self._mapping or item in _DYNAMIC_MODELS or item in self._extra_content
...
def register_model(model_type: str, module_path: str, class_name: str, exist_ok: bool = False):
    """
    ...
    if model_type in MODEL_MAPPING_NAMES and not exist_ok:
        raise ValueError(f"Core model '{model_type}' cannot be overridden. Use exist_ok=True to force.")
...
class AutoModel:
    ...
    def from_pretrained(cls, model_path: str, *args, **kwargs) -> nn.Module:
        ...
        Args:
            model_path: Can be:
                - HuggingFace model name (e.g., 'your-org/auden-zipformer-base')
