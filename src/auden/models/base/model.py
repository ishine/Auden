"""
Example model implementation for auden models.

This module provides an example implementation showing how models should be structured
in the auden framework. It serves as a reference/template, not as a base class
to inherit from.
...
Registering custom models/configs (for AutoConfig/AutoModel):
    The project provides dynamic registries in ``auden.auto`` so your custom
    ...
    3) Register the pair with the provided helpers
        ```python
        from auden.auto.auto_config import register_config, AutoConfig
        from auden.auto.auto_model import register_model, AutoModel
...
class BaseModel(nn.Module):
    """
    Example/reference implementation showing how auden models should be structured.

    This class serves as a template and reference for implementing models in the auden framework.
