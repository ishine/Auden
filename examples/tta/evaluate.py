"""TTA decoding/evaluation script.

Supports ASR-RNNT, ASR-AED, and AST-AED style inference modes, writing
ASR WER-style results or BLEU results depending on the task.
"""
...
import torch
import yaml
from data_module import Speech2TextDataset
...
def get_test_dataloaders(cfg):
    ...
    for test_set in test_data_config:
        ...
        if test_task not in ["transcribe", "translate"]:
...
def main(cfg: DictConfig):
    ...
    for test_set_name, test_dl, test_task in zip(test_sets, test_dls, test_tasks):
        if test_task == "transcribe":
            res_dir = Path(cfg.exp_dir) / "asr_greedy_search"
        elif test_task == "translate":
            res_dir = Path(cfg.exp_dir) / "ast_beam_search"
        ...
        for batch_idx, batch in enumerate(test_dl):
            ...
            if test_task == "transcribe":
                hyps_dict = model.generate(
                    input=(feature, feature_lens), task="transcribe"
                )
                hyps = hyps_dict["hypotheses"]
                texts = batch["supervisions"]["source_text"]
            elif test_task == "translate":
                target_language = batch["supervisions"]["target_language"]
                hyps_dict = model.generate(
                ...
                hyps = hyps_dict["hypotheses"]
                texts = batch["supervisions"]["target_text"]
            else:
                raise ValueError(f"Unsupported task: {test_task}")
