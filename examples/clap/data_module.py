"""Audio-Text (CLAP) example datamodule built on BaseLhotseDatamodule.
...
- If multiple supervisions exist, we keep only the first one. If that caption field contains a
  list, we emit multiple caption entries (flattened) aligned to those cuts; evaluation can then
  repeat audio features accordingly for multi-caption scoring.
...
class AudioCaptionDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, cuts):
        ...
        # Collect captions from ALL supervisions and flatten
        captions = []
        for cut in cuts:
            has_caption = False
            for supervision in cut.supervisions:
                ...
                if isinstance(cap_val, (list, tuple)):
                    captions.extend(self._text_preprocess(str(c)) for c in cap_val)
                else:
